# electromagnetic-radiation.md
# https://youtu.be/FWCN_uI5ygY?si=Tq0UgfI7Cr0kG-2x


0:00
- [Instructor] In the modern world,
0:02
we humans are completely surrounded
0:03
by electromagnetic radiation.
0:06
Have you ever thought of the physics behind
0:08
these traveling electromagnetic waves?
0:11
The great scientist, Heinrich Hertz,
0:13
was the first man to transmit
0:14
and detect electromagnetic waves.
0:17
In his famous experiment,
0:18
a high voltage current was applied to the two ends
0:21
of two metal wires, which generated a spark
0:24
in the gap between them.
0:25
This spark resulted in the radiation
0:27
of electromagnetic waves.
0:29
Those electromagnetic waves traveled through the air
0:32
and created a spark in a metal coil
0:34
located over a meter away.
0:36
If you had placed an LED in that gap,
0:39
the bulb would have glowed.
0:41
This was a clear case of electromagnetic wave propagation
0:44
and detection.
0:46
However, before Hertz, the brilliant mathematician,
0:48
James clerk Maxwell, had already laid out
0:51
the foundations for electromagnetic radiation
0:53
by formulating for mathematical equations.
0:59
However, these equations and the Hertz experiment
1:01
raised a question,
1:03
how do electromagnetic fields detach themselves
1:05
from wires and propagate through a space?
1:08
More specifically, what we need
1:10
is a traveling electromagnetic wave
1:12
and not a fluctuating one.
1:14
Let's explore this logically.
1:18
Consider an electric charge,
1:20
which is moving at a constant speed.
1:22
The electric field around it is shown.
1:25
Now imagine for a fraction of a second it accelerates
1:28
after that, it continues its uniform motion
1:31
at a higher speed.
1:33
What we need to understand
1:34
is the effect of this acceleration on the electric field.
1:38
The interesting thing is that the information
1:41
does not travel at an infinite speed,
1:43
instead, it travels at the speed of light.
1:48
Similarly, the information about the sudden variation
1:51
of velocity of the charge does not get conveyed
1:54
to the whole electric field region.
1:57
The field near it knows about it,
1:58
but the field far away still has no idea
2:01
that the charge has accelerated
2:02
and it is still in the old state.
2:05
Let's separate out these regions
2:07
with the help of two circles.
2:09
Since the electric field cannot break
2:11
the field between these distances must transition.
2:14
This transition field is known as a kink.
2:17
The kink moves or radiates outwards at the speed of light.
2:21
To show the kink animation in a clear way,
2:24
let's move the camera along with the charge.
2:27
We can say here that the acceleration of the charge
2:30
has caused an electromagnetic disturbance
2:32
or electromagnetic radiation.
2:36
Based on this understanding, we will be able to understand
2:39
the most important experiment
2:40
in the field of antenna technology,
2:42
the oscillating electric dipole.
2:44
The interesting fact about this simple oscillating dipole
2:47
is that it produces electromagnetic radiation
2:50
in a perfectly sinusoidal manner.
2:52
Let's see how it is achieved.
2:54
Before getting into the electromagnetics,
2:56
let's understand how velocity
2:58
and acceleration vary in this simple case.
3:00
It is clear that at both ends the velocities should be zero
3:04
and in the middle the velocity should be at the maximum.
3:07
This means that this is a case of continuous acceleration
3:11
and deceleration.
3:12
The electric field pattern is drawn here
3:15
when the chargers are far apart,
3:16
and when the velocity is zero.
3:18
In order to have a better understanding,
3:21
let's examine one of the electric field lines.
3:24
Let's observe the electric field line at t by eight.
3:28
You can see that the electric field line is deformed.
3:31
The reason for this deformation is simple.
3:33
This time period is the region
3:35
with the highest acceleration.
3:36
As we saw earlier, accelerating
3:38
or decelerating charges cause kinks in the electric field.
3:42
In short, the old electric field
3:44
does not get adjusted to the new field very well.
3:47
This deformation is continuous
3:49
since there is continuous acceleration in the charge.
3:53
When two charges meet at the central point,
3:55
the deformed line also meets there.
3:58
After that, it detaches and radiates.
4:01
This radiation travels at the speed of light.
4:04
If you applied an electric field intensity variation
4:07
with respect to length,
4:08
you can see that the radiation we have produced
4:10
is perfectly sinusoidal in nature.
4:12
Please note that this varying electric field
4:15
will automatically generate a varying magnetic field
4:18
perpendicular to it.
4:20
Now let's have a look at how this applies to an antenna.
4:23
A time varying voltage is applied to the metal wire is shown
4:27
due to the effect of the voltage
4:29
the electrons will be displaced from right to left
4:31
and create positive and negative charges.
4:34
With a continuous variation of voltage,
4:36
the positive and negative charges
4:38
will shuttle back and forth in the wire.
4:40
The simple arrangement is known as a dipole antenna.
4:44
The dipole antenna produces the same radiation
4:46
as we saw in the previous section.
4:49
In this case, the antenna works as a transmitter.
4:52
The frequency of the transmitted signal will be the same
4:55
as the frequency of the applied voltage signal.
4:59
The same antenna can act as a receiver
5:01
if the operation of the antenna is reversed.
5:04
When propagating electromagnetic waves strike the antenna,
5:07
the oscillating fields of waves create positive
5:10
and negative charges at the ends of the antenna.
5:13
The varying charge accumulation
5:15
means a varying voltage signal is produced
5:17
at the center of the antenna.
5:19
This voltage signal is the output
5:22
when the antenna works as a receiver.
5:24
We can note here that for perfect transmission or reception,
5:27
the length of the antenna should be half of the wavelength.
5:30
This is the first antenna design criteria
5:33
for proper reception or transmission.
5:35
The second most important design criteria
5:38
is a term called impedance matching.
5:40
Perfect impedance matching
5:42
will make sure that the waves are radiated
5:44
in the most efficient way.
5:47
When an alternating current passes through a circuit,
5:49
it faces opposition from the combined effects of resistance,
5:53
inductance and capacitance.
5:54
This combined effect is known as impedance.
5:57
According to the maximum power transfer theorem,
6:00
to transfer the maximum amount of power
6:02
the load impedance should match with the source impedance.
6:07
For further understanding,
6:08
let's take an example of a circuit
6:10
containing an alternator as a source
6:12
and a motor bulb, et cetera, as a load.
6:16
In this setup to achieve maximum power transfer
6:18
from alternator to the load,
6:20
the impedance of the load must match
6:22
with the impedance of the alternator.
6:27
A similar impedance balance is required
6:29
in the case of an antenna system.
6:31
Since an antenna works on high frequency signals,
6:34
the impedance of the transmission lines
6:36
also becomes important.
6:38
Hence to achieve maximum power,
6:40
the impedance of an antenna should match
6:42
to the impedance of the source
6:43
and transmission line as well.
6:46
If the impedances do not match,
6:48
some portion of the power
6:49
would be reflected back to the source
6:51
instead of radiating outwards from the antenna.
6:55
A free space has an impedance value of 377 ohms.
6:59
In a parabolic antenna, a wave guide is used
7:02
as a transmission line,
7:03
which has a different impedance value from the free space.
7:07
That's why a feedhorn is also included
7:09
in a parabolic antenna.
7:11
This way, the impedance of the wave guide
7:13
is matched with the impedance of the free space
7:16
so that the EM waves can be received properly.
7:20
We hope the concept