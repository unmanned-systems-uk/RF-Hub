...
| `polarisation-mismatch.html` | ✅ **BUILT.** TX/RX polarisation mismatch interactive. TX antenna transmits; RX antenna has a rotation slider (0°–360°). Oscilloscope shows received waveform amplitude changing as cos θ. FFT graph shows peak power dropping. Mode toggle: linear TX vs circular (RHCP) TX — circular mode shows constant 3 dB penalty regardless of RX rotation. FFT rendering matches radiation-3d-v5.html style. | L5 (primary interactive, Section 4) |
| `df-hunt.html` | ✅ **BUILT.** Unit 1 capstone DF game. 2D overhead view. Hidden TX at random position. Two loop antennas at known positions. Student rotates each loop to find the null bearing (two independent FFT graphs show signal strength). Intersection of the two null bearings gives the TX position. "Found" button checks tolerance. "New Game" randomises TX. | L5 (capstone, Section 7) |
